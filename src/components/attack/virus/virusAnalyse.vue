<template>
    <div class="conBox">
        <div class="VAtop structure boxes">
            <h6>病毒事件汇总</h6>
            <div class="mainList boxContent">
                ...
                <div id="virusList">
                    <el-table :data="virusList" height="100%"
        ...
        <div class="VAmiddle structure">
            <div class="VAleft boxes left">
                <h6>病毒分布</h6>
                <div class="mainBar boxContent echartBox">
                    <span class="label">时间：</span>
                    <el-select v-model="barTimeValue" filterable placeholder="请选择"
                                @change="getLimitTime(barTimeValue, virusKind)">
                    ...
                    <el-date-picker v-model="barDateValue" type="datetimerange"
                                    ...
                                    @change="selectLimitTime(barDateValue, virusKind)">
                    </el-date-picker>
                    <div id="virusKind" class="echart"></div>
                    <div id="tip" class="tip"></div>
                </div>
            </div>
            <div class="VAright boxes right">
                <h6>病毒事件趋势</h6>
                <div class="mainLine boxContent echarBox">
                    <span class="label">时间：</span>
                    <el-select v-model="lineTimeValue" filterable placeholder="请选择"
                                @change="getLimitTime(lineTimeValue, virusLine)">
                    ...
                    <el-date-picker v-model="lineDateValue" type="datetimerange"
                                    ...
                                    @change="selectLimitTime(lineDateValue, virusLine)">
                    </el-date-picker>
                    <div id="virusLine" class="echart"></div>
                </div>
            </div>
        </div>
        <div class="VAbottom structure boxes">
            <h6>被攻击IP TOP10</h6>
...
export default {
    ...
    mounted(){
        // 绘制攻击分类图表
        this.initEchartData(this.virusKind)
        // 绘制攻击趋势图表
        this.initEchartData(this.virusLine)
        // 绘制被攻击目的ip趋势图表
    ...
    methods:{
        ...
        // 制作攻击分类图表
        drawVirusKind(obj){
            let Mychart = this.$echarts.init(document.getElementById("virusKind"))
            var seriesData = []
            var legendData = []
        ...
        // 制作攻击趋势图表
        drawVirusLine(obj, start, end){
            let Mychart = this.$echarts.init(document.getElementById("virusLine"))
            let option = {
                color: ["#bf2b27","#fbac47","#fdfe00","#29a7e4","#2a3ac1","#00cd64","#009a00","#305d18"],
                tooltip: {
                    trigger: "axis",
                ...
                this.$axios.post(g.NEW_URL + "/query_esdata?index=ids_av&key=idssyslog_type")
                    .then((res) => {
                        ...
                        if(parseInt(diff/(60*24)) >= 7){
                            this.drawVirusLine(newData, 0, 1)
                        }else if(parseInt(diff/60) >= 12){
                            this.drawVirusLine(newData, 0, 10)
                        }else{
                            this.drawVirusLine(newData, 0, 100)
                        }
                    }).catch((err) => {
                        console.log(err)
                    })
        ...
        initVirList(){
            ...
            this.$axios.post(g.NEW_URL + "/query_esdata?index=ids_av&key=idssyslog_type")
                .then((res) => {
                    this.virusList[0].oneDayTimes = res.data[0].count
                    this.virusList[0].oneWeekTimes = res.data[0].count*7
                    this.virusList[0].oneMonthTimes = res.data[0].count*30
                    this.virusList[0].selTimeTimes = res.data[0].count*30
                }).catch((err) => {
                    console.log(err)
                })
        },
        ...
        selectListLimit(value){
            this.$axios.post(g.NEW_URL + "/query_esdata?index=ids_av&key=idssyslog_type")
                .then((res) => {
                    this.virusList[0].selTimeTimes = res.data[0].count*40
                }).catch((err) => {
                    console.log(err)
                })
        },
...
<style>
/* 页面分区区块样式 */
.virusPage .Vcontent .VAmiddle,
.virusPage .Vcontent .VAbottom{
    height:300px;
}
.virusPage .Vcontent .VAtop{
    min-height:180px;
}
</style>