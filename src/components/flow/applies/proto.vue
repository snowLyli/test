<template>
    <div class="conBox">
        <div class="PPtop structure">
            <div class="PPleft boxes">
                <h6>实时流量分布</h6>
                <div id="realProKind"></div>
            </div>
            <div class="PPcenter boxes">
                <h6>累计流量分布</h6>
                <div id="allProKind"></div>
            </div>
            <div class="PPright boxes">
                <h6>实时连接数分布</h6>
                <div id="linkProKind"></div>
            </div>
        </div>
        <div class="PPmiddle structure boxes">
            <h6>流量趋势</h6>
            <div class="mainLine boxContent">
                <span class="label">时间:</span>
                <el-select v-model="lineTimeValue" filterable placeholder="请选择"
                            @change="selectLimitTime(lineTimeValue, lineData, flowTypeValue, 'proLine')">
                ...
                <el-date-picker v-model="lineDateValue" type="datetimerange"
                                ...
                                @change="selectLimitDate(lineDateValue,  lineData, flowTypeValue, 'proLine')">
                </el-date-picker>
                <span class="label">流量类型:</span>
                <el-select v-model="flowTypeValue" filterable placeholder="请选择"
                            @change="selectFilter(lineTimeValue, lineDateValue, lineData, flowTypeValue, 'proLine')">
        ...
        <div class="PPbottom structure boxes">
            <h6>应用分类统计详细列表</h6>
...
export default {
    ...
    mounted(){
        this.getPieData("realProKind", "实时")
        this.getPieData("allProKind", "累计")
        this.getPieData("linkProKind", "连接数")
        // 曲线图
        // 曲线图
        this.drawLine(this.lineData, "proLine", 0, 100)
        // 详细信息列表
        this.initList(this.proList)
    },
    methods: {
        random(min, max){
        ...
        getSummaries(param){
            ...
            this.initProList.map((item, index) => {
...
/* 图表容器样式 */
.PPtop .PPleft #realProKind,
.PPtop .PPcenter #allProKind,
.PPtop .PPright #linkProKind,
.PPmiddle .mainLine #proLine{
    width:100%;
    height:85%;
}

/* 列表样式 */
.PPottom .kindStaList .detailList{
    margin-bottom:10px;
}
/* 搜索框样式 */
.PPbottom .kindStaList .searchBox{
    width:100%;
    height: 30px;
    border-radius:3px;
    margin-bottom:10px;
}
.PPbottom .kindStaList .searchBox .applyBlock{
    height: 30px;
    text-align:center;
    float:right;
}
