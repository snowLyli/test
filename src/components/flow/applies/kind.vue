<template>
    <div class="conBox">
        <div class="KPtop structure">
            <div class="KPleft boxes">
                <h6>实时流量分布</h6>
                <div id="realKindKind"></div>
            </div>
            <div class="KPcenter boxes">
                <h6>累计流量分布</h6>
                <div id="allKindKind"></div>
            </div>
            <div class="KPright boxes">
                <h6>实时连接数分布</h6>
                <div id="linkKindKind"></div>
            </div>
        </div>
        <div class="KPmiddle structure boxes">
            <h6>流量趋势</h6>
            <div class="mainLine boxContent">
                <span class="label">时间:</span>
                <el-select v-model="lineTimeValue" filterable placeholder="请选择"
                            @change="selectLimitTime(lineTimeValue, lineData, flowTypeValue, 'kindLine')">
                ...
                <el-date-picker v-model="lineDateValue" type="datetimerange"
                                ...
                                @change="selectLimitDate(lineDateValue,  lineData, flowTypeValue, 'kindLine')">
                </el-date-picker>
                <span class="label">流量类型:</span>
                <el-select v-model="flowTypeValue" filterable placeholder="请选择"
                            @change="selectFilter(lineTimeValue, lineDateValue, lineData, flowTypeValue, 'kindLine')">
        ...
        <div class="KPbottom structure boxes">
            <h6>应用分类统计详细列表</h6>
            <div class="kindStaList boxContent">
                ...
                <div class="detailList logTable">
                    <el-table :data="kindList" stripe style="width: 100%" border
                        ...
                        <el-table-column label="应用名称">
                            <template slot-scope="props">
                                <span @click="goDetail(props.row.label)" style="cursor:pointer;">{{props.row.label}}</span>
                            </template>
                        </el-table-column>
                        ...
                        <el-table-column prop="linkCount" label="连接数(次)" sortable></el-table-column>
                    </el-table>
                </div>
            </div>
        </div>
...
export default {
    ...
    mounted(){
        // 饼图
        this.getAllUrl()
        this.getPieData("realKindKind", this.realKindUrl)
        this.getPieData("allKindKind", this.allKindUrl)
        this.getPieData("linkKindKind", this.linkKindUrl)
        // 获取流量类型选择框数据
        this.getFlowType()
        // 曲线图
        this.drawLine(this.lineData, "kindLine", 0, 100)
        // 详细信息列表
        this.initList(this.initKindList, this.kindList)
    },
    methods: {
        random(min, max){
        ...
        // 给初始数据添加单位
        initList(initOrigin, origin){
            this.$axios.post(g.NEW_URL + "/query_esdata?index=dpi_log_app&key=AppClass,RxBytes,TxBytes,RxPkts,TxPkts,Session")
                       .then((res) => {
                            res.data[1].context.map((item, index) => {
                                origin.map((itm, idx) => {
                                    if(item.AppClass == itm.name){
                                        itm.entrySpeed+=Number(item.RxBytes)
                                        itm.outSpeed+=Number(item.TxBytes)
                                        itm.allEntrySpeed+=Number(item.RxPkts)
                                        itm.allOutSpeed+=Number(item.TxPkts)
                                        itm.linkCount+=Number(item.Session)
                                    }
                                })
                                initOrigin.map((itm, idx) => {
                                    if(item.AppClass == itm.name){
                                        itm.entrySpeed+=Number(item.RxBytes)
                                        itm.outSpeed+=Number(item.TxBytes)
                                        itm.allEntrySpeed+=Number(item.RxPkts)
                            ...
                            origin.map((item, index) => {
                                item.entrySpeed = OtherFunction.addUnit(item.entrySpeed, this.speed, "flow")
                                item.outSpeed = OtherFunction.addUnit(item.outSpeed, this.speed, "flow")
                                item.allEntrySpeed = OtherFunction.addUnit(item.allEntrySpeed, this.allFlow, "flow")
                                item.allOutSpeed = OtherFunction.addUnit(item.allOutSpeed, this.allFlow, "flow")
                                item.linkCount = item.linkCount
                            })
                       }).catch((err) => {
                           alert("数据请求失败")
                       })
        ...
        // 所有url
        getAllUrl(){
            this.realKindUrl = g.NEW_URL + "/query_esdata?index=dpi_log_app_update&key=AppClass"
            this.allKindUrl = g.NEW_URL + "/query_esdata?index=dpi_log_app&key=AppClass"
            this.linkKindUrl = g.NEW_URL + "/query_esdata?index=dpi_log_app_update&key=AppClass,Session"
        },
                       ...
                       .then((res) => {
                            ...
                            if(id == "linkKindKind"){
                                ...
                                this.pieData.map((item, index) => {
                                    ...
                                    this.linkData.push(itemObj)
                                })
                                res.data[1].context.map((item, index) => {
                                    this.linkData.map((itm, idx) => {
                                        if(item.item == itm.name){
                                            itm.value+=item.Session
                                        }
                                    })
                                })
                                this.linkData.map((item, index) => {
                                    resValue.push(item.value)
                                    resName.push(item.label)
                                })
                            }else{
                            ...
                            var seriesName = []
                            // 将数据排序，提取出最大的几个数及其下标
                            var needObj = OtherFunction.arrRank(resValue, resValue.length)
                            // 其他项数据总和
                            var other = 0
        ...
        getSummaries(param){
            ...
            this.initKindList.map((item, index) => {
                sums[1]+=item.entrySpeed
                sums[2]+=item.outSpeed
                sums[3]+=item.allEntrySpeed
                sums[4]+=item.allOutSpeed
                sums[5]+=item.linkCount
...
/* 图表容器样式 */
.KPtop .KPleft #realKindKind,
.KPtop .KPcenter #allKindKind,
.KPtop .KPright #linkKindKind,
.KPmiddle .mainLine #kindLine{
    width:100%;
    height:85%;
}

/* 列表样式 */
.KPottom .kindStaList .detailList{
    margin-bottom:10px;
}
/* 搜索框样式 */
.KPbottom .kindStaList .searchBox{
    width:100%;
    height: 30px;
    border-radius:3px;
    margin-bottom:10px;
}
.KPbottom .kindStaList .searchBox .applyBlock{
    height: 30px;
    text-align:center;
    float:right;
}
